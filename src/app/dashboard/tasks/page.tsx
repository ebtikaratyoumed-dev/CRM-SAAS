  ...
  if (loadingData) {
    return (
      <div className="p-8 text-center text-slate-400">
        ...
        <p className="font-medium">Chargement des taches...</p>
      </div>
    );
  }
    ...
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Gerez vos interventions et le suivi des travaux.
          </p>
        </div>
      </div>
      ...
      {currentTab === 'list' ? (
        <Tabs defaultValue="tous" className="w-full">
          <div className="flex items-center justify-between mb-6 gap-4">
            <TabsList className="bg-slate-900/50 border border-slate-800 p-1">
              ...
              <TabsTrigger value="A faire" className="data-active:bg-slate-800">A faire</TabsTrigger>
              <TabsTrigger value="En cours" className="data-active:bg-slate-800">En cours</TabsTrigger>
              <TabsTrigger value="Termine" className="data-active:bg-slate-800">Terminees</TabsTrigger>
            </TabsList>
          ...
          {['A faire', 'En cours', 'Termine'].map(status => {
            // Map statuses to French as defined in the database/UI
            const dbStatus = status === 'Termine' ? 'Terminé' : status === 'A faire' ? 'À faire' : status;
            const tabFilteredTasks = filteredTasks.filter((t: any) => t.status === dbStatus) || [];
        ...
        <Card className="max-w-2xl mx-auto bg-slate-900/50 border-slate-800">
          <CardHeader>
            <CardTitle>Creation de Tache</CardTitle>
            <CardDescription>Assignez une nouvelle tache a un membre de l'equipe.</CardDescription>
          </CardHeader>
          <CardContent>
            <TaskForm 
              projects={projects || []} 
